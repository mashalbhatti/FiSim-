...
namespace FiSim.GUI {
    internal class SimulationManager {
        ...
        internal void RunProgramSimulation(OutputConsoleWriter outputWriter, OnGlitchSimulationCompletedCallback onGlitchSimulationCompletedCallback, string filePath)
        {
            if (!File.Exists(Config.EngineExe))
            {
                throw new Exception("File not found: " + Config.EngineExe);
            ...
            //Console.WriteLine(Config.EngineExe); 
            string[] myArguments = { Config.ProjectName + " P", filePath };
            ...
            var process = new Process
            {
                StartInfo = {
                    FileName = Config.EngineExe,
                    //Arguments = Config.ProjectName + " P",
                    Arguments =  String.Join(",", myArguments), 
                    UseShellExecute = false,
                    RedirectStandardOutput = true,
                    RedirectStandardError = true,
                    CreateNoWindow = true 
